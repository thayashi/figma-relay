...
When the user says any of these phrases, immediately follow the linked skill — no exploration needed.

| Wake Word | Action | Skill File |
|---|---|---|
| **"figma"** or **"bridge"** | Connect to Figma via HTTP Bridge (localhost:3056). Check `/status` first, then use `/command` or `/execute`. | `.claude/skills/figma-bridge.md` |

**How it works**: If the user's message contains a wake word above, read the linked skill file and follow it as your first action. Do not search the codebase or ask clarifying questions about *how* to connect — the skill has everything you need.
...
### Mandatory: Design Review After Figma Changes

After **any** visual changes to a Figma file (creating frames, modifying layouts, updating styles, importing components), you **MUST** follow the `.claude/skills/design-review.md` skill before reporting the task as complete. This is not optional.

The workflow is:
1. Screenshot every affected screen/frame at scale 2
2. Review for layout, typography, visual, and structural issues
3. Fix any problems found, then re-screenshot to verify
4. Show the final screenshots to the user
5. Save a Figma version for significant changes

Never skip this step, even if you are confident the code is correct. Silent failures in the Figma Plugin API are common (see `figma-structure.md` gotchas).
